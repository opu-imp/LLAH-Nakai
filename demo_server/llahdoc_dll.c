...
// 特徴点抽出
{
	...
	img = GetConnectedImage2( fname, CONST_HASH_MODE, eHashGaussMaskSize );	// 結像画像を作成
...
//	MakeNearsFromCentres( ps, num, &nears );
	if ( num >= kMinPoints ) {	// 特徴点数が最小値以上
		...
		// 検索
		res = RetrieveNN5Cor2( ps, areas, num, nears, img_size, score, pcor, &pcornum, &disc, eProp, reg_nums, hash, hash2, hash3 );	// 検索のみ
