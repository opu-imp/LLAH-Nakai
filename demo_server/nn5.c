...
//	eProp = 0.0;	// スコア＝得票数にする
	for ( i = 0; i < eDbDocs; i++ ) {
		score[i] = (int)((double)score[i] - (double)reg_nums[i] * prop);	// CBDARでの補正
	}
	// 得票数最大のものを探す
	for ( i = 0; i < eDbDocs; i++ ) {
		if ( score[i] > max_score ) {
...
// ハッシュから検索する（すべての文書への対応点を返すバージョン）
{
	int i, j, max_score = 0, max_doc = 0, second_score = 0, second_doc = 0;
	int start, end;
	char line[kMaxLineLen];
...
// 特徴点抽出モード3（公開用に改変）
{
	...
	for ( i = 0; i < eDbDocs; i++ ) {
		...
		if ( IsDat( files[i] ) ) {	// datファイルの場合
			...
			img = GetConnectedImage2( files[i], CONST_HASH_MODE, eHashGaussMaskSize );	// 結像画像を作成
//			OutPutImage( img );
...
// 09/01/06 デュアルハッシュに対応するため，いろいろと引数で指定するようにした
{
	int i, j, n;
	FILE *fp, *fp_pn;
    int num = 0, **nears = 0;
	...
	sprintf( search_path, "%s*.%s", hash_src_dir, hash_src_suffisx );	// 検索パスの作成
	sprintf( pdat_fname, "%s%s", dirs_dir, point_dat_file_name );	// point.datのパスを作成
	if ( ( fp = fopen( pdat_fname, "w" ) ) == NULL ) {
		fprintf( stderr, "error: %s cannot be opened\n", pdat_fname );
		return 0;
	}
	...
	for ( i = 0; i < eDbDocs; i++ ) {
		...
		else			img = GetConnectedImage2( files[i], CONST_HASH_MODE, gaussian_parameter );	// 結像画像を作成
#ifdef	DRAW_FP
