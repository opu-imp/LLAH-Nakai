...
// 引数解析の関数2
{
	...
	// 初期化
#ifdef	WIN
	eEntireMode = USBCAM_SERVER_MODE;	// 引数なしではリアルタイム検索にする
#else
	eEntireMode = RETRIEVE_MODE;	// LINUX版では静止画検索モードをデフォルトにする
#endif
...
#ifndef	WIN	// DOSのmkdirはパスのデリミタが'\\'でないとエラーになるので，Windowsでは無効にした
