...
IplImage *GetConnectedImageCam( IplImage *src, IplImage *inv )
{
	int i;
	static IplImage *gray = NULL, *bin = NULL, *img_hist = NULL;
//	IplImage *inv;
	strHist hist;
    CvMemStorage* storage = cvCreateMemStorage(0);
	CvSeq *contours = 0, *con0;
...
#ifdef	DRAW_HIST
	if ( img_hist == NULL )
		img_hist = cvCreateImage( cvSize( 1000, 200 ), IPL_DEPTH_8U, 1 );
#endif
...
//	point->origin = src->origin;
	...
	cvNot( bin, inv );
#ifdef	DRAW_HIST
	cvFindContours( inv, storage, &contours, sizeof(CvContour), CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cvPoint(0,0) );	// 連結成分を抽出する
	// 連結成分を描画・重心を計算
	InitHist( &hist, 500, 0, 200 );
    for( i = 0, con0 = contours; con0 != 0 ; con0 = con0->h_next ) {
		cvMoments(con0, &mom, 1);
		d00 = cvGetSpatialMoment( &mom, 0, 0 );
		if ( d00 < 2 ) continue;	// 小さすぎる連結成分は除外
		AddDataHist( &hist, sqrt(d00) );
    }
	DrawHist( img_hist, &hist );
	cvShowImage( "Hist", img_hist );
	ReleaseHist( &hist );
	if ( contours )	cvClearSeq( contours );
	cvReleaseMemStorage( &storage );
#endif
// 07/06/07
//	cvAdaptiveThreshold( gray, bin, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 31, 10 );
//	cvNot( bin, inv );
//	cvSmooth( bin, gray, CV_GAUSSIAN, 7, 0, 0 );
	// 07/06/07
//	cvSmooth( bin, gray, CV_GAUSSIAN, 3, 0, 0 );
	// 08/08/25 多言語
	cvSmooth( bin, gray, CV_GAUSSIAN, eCamGaussMaskSize, 0, 0 );
