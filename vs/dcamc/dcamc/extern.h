...
Extern int eExperimentMode;	// 実験モード（処理時間等を出力）
...
Extern int eCamGaussMaskSize;	// 特徴点抽出のガウシアンフィルタのサイズ
Extern int eCamGaussMaskSize1;	// 特徴点抽出のガウシアンフィルタのサイズ（文字単位）
Extern int eCamGaussMaskSize2;	// 特徴点抽出のガウシアンフィルタのサイズ（単語単位）
Extern int eCaptureWidth;	// キャプチャウィンドウの幅（0ならリサイズしない）
Extern int eThumbWidth;	// サムネイルウィンドウの幅（0ならeThumbScaleを使う）
...
Extern int eFocusCoupledGaussian; // ガウシアンフィルタのパラメータをフォーカス連動にする

Extern int eDualHash;	// デュアルハッシュモード
