...
int MergeImage4( IplImage *img1, IplImage *img2, IplImage *img3, IplImage *img4, IplImage *merge )
// img1とimg2とimg3とimg4を並べた画像をmergeに格納する
// PRMU0902（多言語）のビデオ用
{
	int i, j, mw, mh, orig_x = 0, orig_y = 0;
	char val[3];
	...
	for ( j = 0; j < mh; j++ ) {
		for ( i = 0; i < mw; i++ ) {
			memset( val, 0, 3 );
			if ( i >= img1->width ) { // 画像1の幅を越える
				if ( j >= img2->height + img3->height )	// 画像2の高さ＋画像3の高さを超える
					GetPix( img4, i - img1->width, j - img2->height - img3->height, val );	// 画像4のピクセルを見る
				else if ( j >= img2->height )	// 画像2の高さを越える
					GetPix( img3, i - img1->width, j - img2->height, val );	// 画像3のピクセルを見る
				else
					GetPix( img2, i - img1->width, j, val );	// 画像2のピクセルを見る
			} else {
				GetPix( img1, i, j, val );	// 画像1のピクセルを見る
			}
			SetPix( merge, orig_x + i, orig_y + j, val );
		}
	}
//	cvRectangle( merge, cvPoint( orig_x + img1->width, orig_y + img2->height ), cvPoint( orig_x + img1->width + img3->width - 1, orig_y + img2->height + img3->height - 1 ), CV_RGB(0,0,0), 1, CV_AA, 0 );
	cvLine( merge, cvPoint( orig_x + img1->width, orig_y + img2->height ), cvPoint( merge->width, orig_y + img2->height ), CV_RGB(0,0,0), 1, 8, 0 );
	cvLine( merge, cvPoint( orig_x + img1->width, orig_y + img2->height + img3->height ), cvPoint( merge->width, orig_y + img2->height + img3->height ), CV_RGB(0,0,0), 1, 8, 0 );
